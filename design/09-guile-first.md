...
- **§4 状态模型已实现**：`(cind state)` 状态根落地，`(cind command)` 的 11 张 per-host 表已收编，测试与按键路径实测均无回归。其余 9 个模块的 30 张表待迁移（P1 的机械部分）。
- **§5 事务边界与 §6 containment 仍是草案**，在动到 buffer/window registry（P2）之前需定稿——命令层的状态迁移尚未触及这两条立场。
...
**首个迁移（已完成）**：`(cind command)` 的 11 张表全部收编——2 张运行时状态（command-feedback、command-prefix）+ keymap roots + 8 个表现层策略（display/modeline/chrome/theme/style/motion/metrics/typography）。8 个策略原本各有一对"配置/解析"函数，每对都重复"取表→未配置则报错→调用"的样板，收编后塌缩为 8 行声明加各自的薄封装。按键路径实测无回归（中位 0.105 ms，基线 0.126 ms，噪声内）。
...
- **P1**：Guile 状态根骨架**（已完成）** + 剩余 9 个模块的 per-host 表收编（机械）+ 已多半在 Guile 的子系统整块收编（workbench 元数据、jump walk、location 当前引用、completion 会话）——删除对应 native 半边与桥原语，验证"删得动"。

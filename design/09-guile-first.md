...
- **P2（进行中）**：placement 已完成——display 请求原先逐窗口回调 Guile 取 role/pinned/slot、再把这些 facts 传回 Guile 做决策；现在 facts 只携带 native 真值（intent、origin、active、layout 窗口序），策略直接读 `(cind workbench)` 自己的状态，plan 校验只查目标窗口。同批删掉三处"进 Guile 建向量再丢弃"的存在性探针。**剩余**：buffer/window registry 元数据、interaction、session 序列化、EditorApplication 编排改薄转发。
  - **session 序列化的形态判断**（尚未动手）：`capture_workbench_session` 是 114 行 C++ 逐项回调 Guile 取 name/scope/MRU 再与 native 数据（layout 树、jump 图、buffer resource）拼接——两侧都不独占，谁来构建都要读对方。搬到 Guile 需要新增 layout 树与 jump 图的导出原语，并在 Scheme 重写带 `std::quoted`/浮点比例的定位文本格式；风险在格式保真（浮点往返与引号），收益是删除约 700 行 native。建议独立一轮做，先立"序列化字节与旧实现逐字节一致"的验收。
  - **buffer 元数据的形态判断**：`.name()`/`resource_uri()` 等在 native 侧有上百处消费方（compose、modeline facts、LSP 同步、save）。整体搬进 Guile 会制造 **C++→Scheme 的反向往返**，正是 §0.2 的病。可行路径是先把 native 消费方改为"取值作参数"，再迁字段——不宜直接搬。

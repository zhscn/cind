...
void publish_test_frame(InspectionHub& hub, bool row_overflow = false,
    ...
    EditorStateSnapshot editor{
        ...
        .buffers = {{.buffer_slot = 0,
                     ...
                     .completion_providers = {"clangd", "word", "path"},
                     .location_count = 0,
                     .diagnostic_count = 3,
                     .diagnostic_errors = 1,
            ...
            {{.workbench = {.slot = 0, .generation = 1},
              .nodes = {{.id = 1,
                        ...
                        {.id = 2,
                         ...
                         .created_at = 2,
                         .last_visit = 5}},
              .edges = {{.from = 1, .to = 2, .kind = "manual", .at = 6, .persistent = true}},
              .walks = {{.window = {.slot = 0, .generation = 1}, .entries = {1, 2}, .cursor = 0}}}},
...
TEST_CASE("inspection snapshot exposes model, scene, render, and event state") {
    ...
    CHECK(buffers.payload.find("\"completion_providers\":[\"clangd\",\"word\",\"path\"]") !=
          std::string::npos);

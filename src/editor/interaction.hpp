...
public:
    using Complete = std::function<std::vector<InteractionCandidate>(CommandContext&,
                                                                      std::string_view)>;
...
// Owns the active prompt or picker independently of any frontend. Providers
// produce semantic candidates; frontends only render the state and translate
...
public:
    ...
    std::expected<void, std::string> start(InteractionRequest request,
...
private:
    ...
    static std::vector<InteractionCandidate>
    rank(std::vector<InteractionCandidate> candidates, std::string_view query);

...
BasicEditorCommands::BasicEditorCommands(EditorRuntime& runtime, EditSession& session,
                                         BasicEditorCommandHooks hooks)
    : runtime_(&runtime), session_(&session), hooks_(std::move(hooks)) {
    ...
    define("edit.undo", [this](const CommandInvocation&) {
    ...
    define("edit.redo", [this](const CommandInvocation&) {
        reset_preferred_column();
    ...
    define("cursor.line-start", [this](const CommandInvocation&) { move_line_boundary(false); });
    define("cursor.line-end", [this](const CommandInvocation&) { move_line_boundary(true); });
    define("cursor.next-line",
           [this](const CommandInvocation& invocation) { move_vertical(1, false, invocation); });
    define("cursor.previous-line",
           [this](const CommandInvocation& invocation) { move_vertical(-1, false, invocation); });
    define("cursor.page-down",
           [this](const CommandInvocation& invocation) { move_vertical(1, true, invocation); });
    define("cursor.page-up",
           [this](const CommandInvocation& invocation) { move_vertical(-1, true, invocation); });
    define("cursor.forward-character",
           [this](const CommandInvocation& invocation) { move_horizontal(true, invocation); });
    define("cursor.backward-character",
           [this](const CommandInvocation& invocation) { move_horizontal(false, invocation); });
    define("edit.delete-backward", [this](const CommandInvocation&) {
        reset_preferred_column();
        const RevisionId revision = session_->snapshot().revision();
        soft_delete(false);
        if (session_->snapshot().revision() != revision) {
            notify_edited();
        }
    });
    define("edit.delete-forward", [this](const CommandInvocation&) {
        reset_preferred_column();
        const RevisionId revision = session_->snapshot().revision();
        soft_delete(true);
        if (session_->snapshot().revision() != revision) {
    ...
    define("edit.newline", [this](const CommandInvocation&) {
        reset_preferred_column();
        session_->enter();
        notify_edited();
    });
    define("edit.indent", [this](const CommandInvocation&) {
        reset_preferred_column();
        const RevisionId revision = session_->snapshot().revision();
        const IndentDecision decision = session_->indent();
        if (session_->snapshot().revision() != revision) {
            notify_edited();
        }
...
void BasicEditorCommands::reset_preferred_column() {
    session_->view().viewport().preferred_column.reset();
}

void BasicEditorCommands::move_horizontal(bool forward, const CommandInvocation& invocation) {
    reset_preferred_column();
...
void BasicEditorCommands::move_vertical(int direction, bool page,
    ...
    const int delta = static_cast<int>(std::clamp<std::int64_t>(
        ...
        static_cast<std::int64_t>(std::numeric_limits<int>::max())));
    const int current_line = static_cast<int>(text.position(session_->caret()).line);
...
void BasicEditorCommands::move_line_boundary(bool end) {
    reset_preferred_column();
    const DocumentSnapshot snapshot = session_->snapshot();
    const Text& text = snapshot.content();
    const std::uint32_t line = text.position(session_->caret()).line;
    session_->set_caret(end ? text.line_content_end(line) : text.line_start(line));
    notify_caret_moved();
}

void BasicEditorCommands::soft_delete(bool forward) {
    const DocumentSnapshot snapshot = session_->snapshot();
    const Text& text = snapshot.content();
    ...
    if (is_open(character) || is_close(character)) {
        ...
        if (is_close(character) && target.value >= 1 &&
            text.byte_at(TextOffset{target.value - 1}) == partner(character)) {
            session_->erase(TextRange{TextOffset{target.value - 1}, TextOffset{target.value + 1}});
            return;
        }
    ...
    if (character == '"' || character == '\'') {
        ...
        if (other == character) {
            ...
            session_->erase(make_range(start, start + 2));
            return;
        }
        session_->set_caret(forward ? TextOffset{target.value + 1} : target);

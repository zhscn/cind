...
`EditorModel` 提供文档、caret、viewport 和编辑状态。`ui::Scene` 是后端无关的语义帧：
正文和 gutter region 携带 cell primitive 与文档坐标映射，popup、status 和 echo region
携带结构化内容。`ViewTree` 把 region 组织为显式的 Grid、Chrome 和 Overlay 层，绘制与
命中使用相反的遍历方向。
...
- `scene`：cell 网格、cursor、活动文本行、view tree、region 几何和语义内容。Scene

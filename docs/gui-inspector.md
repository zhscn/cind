...
echo region 携带结构化内容。补全文档使用独立的 Documentation overlay region。
`ViewTree` 把 region 组织为显式的 Grid、Chrome 和 Overlay
层，绘制与命中使用相反的遍历方向。
...
`SkiaPresenter` 只清除并重绘损伤逻辑像素矩形；SDL texture 只上传对应的物理
output 像素矩形。SDL renderer 仍将完整 retained texture 合成到当前 swapchain image，
因为 swapchain image 不保留上一帧内容。
分数滚动单独记录为 `render.damage.grid_transform_changed`，只损伤正文 grid，不包含固定
footer。整数 output-pixel 平移时，双 target texture 复制已有像素并只上传暴露条带；
`render.timings.texture_scroll_reused` 和 `texture_copy_pixels` 记录该路径。Inspector 会
read back 实际 renderer 输出并与完整 CPU 参考帧逐像素比较。`render.timings` 还提供各阶段
微秒数、上传字节数，以及每帧 shaped-text cache 的 hit/miss/eviction 计数。
...
prompt 行加候选行让出整行空间（reflow 而非 overlay），不随正文滚动位置移动。Picker
在交互期间保留固定容量，过滤到零项时仍由 popup header 持有输入和 cursor；异步 provider
完成后原子替换上一份完整候选快照。
...
| `editor.completion` | 补全 trigger、generation/revision、anchor/caret/query、选中项、等待中的 native 或 scripted provider，以及按稳定 ID 暴露的候选、resolve 状态、错误和文档；scene popup 同时标明 `band` 或 `completion` presentation |
| `editor.lsp` | project-scoped LSP session 的状态、命令、root、等待中的 request、已同步文档数量、server capabilities 和最近错误 |
| `editor.buffers` | 所有打开 buffer 的资源、major mode、interaction class、初始 InputState、自动补全策略、有效 completion provider、semantic thing 名称与 registry definition、location 数量、diagnostic 总数及 error/warning 数量、view ID、modified、saving 和 active 状态 |
...
`describe-xxx` 和 Scheme evaluation 使用普通的 read-only generated buffer 与
`special-mode`。因此 `*Help*`、`*Scheme Evaluation*` 的 buffer/view/window 归属通过
`editor.buffers`、`editor.windows` 和 `editor.focus` 检查，最终绘制仍通过 `scene` 与
`render.document_layout` 检查；inspector 不维护另一份功能专用状态。
...
cmk run -p gui cind-ui-inspect -- get editor.lsp

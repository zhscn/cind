...
| `editor.completion` | 补全 generation/revision、anchor/caret/query、选中项、等待中的 native 或 scripted provider，以及按稳定 ID 暴露的候选、resolve 状态、错误和文档 |
| `editor.lsp` | project-scoped LSP session 的状态、命令、root、等待中的 request、已同步文档数量、server capabilities 和最近错误 |
| `editor.buffers` | 所有打开 buffer 的资源、major mode、interaction class、初始 InputState、有效 completion provider、semantic thing 名称与 registry definition、location 数量、diagnostic 总数及 error/warning 数量、view ID、modified、saving 和 active 状态 |

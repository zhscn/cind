...
(define (modeline-content host context facts)
  (let* ((resource (vector-ref facts 1))
         (dirty? (vector-ref facts 2))
         (line (vector-ref facts 3))
         (column (vector-ref facts 4))
         (line-count (vector-ref facts 5))
         (revision (vector-ref facts 6))
         (style-origin (vector-ref facts 7))
         (last-key (vector-ref facts 8))
         (input-state (vector-ref facts 9))
         (workbench-name (active-workbench-name host))
         (name (if resource
                   (path-filename host resource)
